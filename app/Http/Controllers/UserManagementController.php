...
class UserManagementController extends Controller
{
    ...
    public function updateWebsiteLink(Request $request)
    {
        $request->validate([
        ...
        return response()->json(['message' => 'Website link updated successfully!']);
    }

    public function list()
    {
        $users = User::select('id', 'name', 'website_link')->where('id',auth()->user()->id);

        return DataTables::of($users)
            ->addColumn('actions', function($user) {
                return '
                <button class="btn btn-warning edit-website-link" data-id="'.$user->id.'">Edit</button>
                <button class="btn btn-danger delete-website-link" data-id="'.$user->id.'">Delete</button>
            ';
            })
            ->rawColumns(['actions'])
            ->make(true);
    ...
    public function editWebsiteLink($id)
    {
        $user = User::find($id);
        return response()->json($user);
    }

    public function deleteWebsiteLink($id)
    {
        $user = User::find($id);
        $user->website_link = null;
        $user->save();

        return response()->json(['message' => 'Website link deleted successfully!']);
    }
    ...
    public function profile()
    {
        ...
        $education=EducationBackground::where('user_id',$profile)->where('is_delete',0)->first();
        $otherQualifications=OtherQualification::where('user_id',$profile)->where('is_delete',0)->get();
        $educationData = [
            'education' => $education,
        ...
        if (empty($employees)) {
            ...
            $familyInformations = FamilyInformation::where('user_id', $profile)->get(); // Fixed the typo
            // return $familyInformations;
            return view('usermanagement.profile_user', compact('information', 'user', 'userInformation', 'familyInformations','profile','experience','educationData')); // Fixed the typo
