...
class EmployeeInformationController extends Controller
{
    public function index(Request $request){
        ...
        $employeeeInformation=EmployeeInformation::where('user_id',auth()->user()->id)->first();
        if ($request->ajax()) {
            $data = EmployeeInformation::
            where('user_id',auth()->user()->id)->where('is_delete',0)
                ->select(['id', 'user_id','name', 'date_of_birth', 'address', 'contact_number', 'email', 'nationality', 'marital_status'])
                ->get();
...
//        return  $employeeeInformation;
        return view('cv.index',compact('users','employeeeInformation'));
    }
